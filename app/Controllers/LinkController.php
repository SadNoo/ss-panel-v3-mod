...
class LinkController extends BaseController
{
    ...
    public static function GetContent($request, $response, $args)
    {
        ...
        switch ($Elink->type) {
            case -1:
                ...
                $newResponse = $response->withHeader('Content-type', ' application/octet-stream; charset=utf-8')->withHeader('Cache-Control', 'no-store, no-cache, must-revalidate')->withHeader('Content-Disposition', ' attachment; filename=black hole.conf');//->getBody()->write($builder->output());
    ...
    public static function GetIosConf($user, $is_mu = 0, $is_ss = 0)
    {
        ...
        $rules = file_get_contents("https://raw.githubusercontent.com/lhie1/tuClub/master/Rule.conf");

        $items = URL::getAllItems($user, $is_mu, $is_ss);
        foreach($items as $item) {
            $proxy_group .= $item['remark'].' = custom,'.$item['address'].','.$item['port'].','.$item['method'].','.$item['passwd'].',http://omgib13x8.bkt.clouddn.com/SSEncrypt.module'.URL::getSurgeObfs($item).',obfs-host=wns.windows.com,tfo=true'."\n";
...
// Auto
loglevel = notify
dns-server = system,208.67.222.222,208.67.220.220
